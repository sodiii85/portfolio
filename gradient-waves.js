/* ============================================================
   GradientWaves — raymarched sine-plasma wave field.

   Vanilla port of the React Bits <GradientWaves /> component
   (https://reactbits.dev). The shaders and option names are
   verbatim; the React lifecycle becomes an init() that returns
   its own teardown, since this project ships plain ES modules
   with no framework.

   Two deliberate deviations from the upstream source:
     · pointer tracking listens on window, not on the canvas, so
       parallax still works while the layer sits behind the page
       with pointer-events: none;
     · prefers-reduced-motion renders a single static frame
       instead of animating, matching the rest of the stylesheet.
   ============================================================ */
...
/**
 * Mount the wave field into `container` (it fills the element).
 * Returns a teardown function, or null when WebGL2 is unavailable —
 * callers can leave the plain background in place in that case.
 */
export function initGradientWaves(container, options = {}) {
  ...
  const horizon = hexToRgb(opts.horizonColor);
  const wave = hexToRgb(opts.waveColor);
  const crest = hexToRgb(opts.crestColor);
  ...
  // Window-level, so parallax survives pointer-events: none on the layer.
  ...
  // Honouring the setting live: stop on request, resume if it is turned off.
  const onMotionPreference = () => (reduceMotion.matches ? tryStop() : tryStart());
...
export default initGradientWaves;
